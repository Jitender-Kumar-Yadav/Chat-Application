...
user_check = False
host = input("Enter the server's IPv4 address. For local server, use 'local'.\n")
if host == "local":
...
while not (host_check and user_check):
    try:
        ...
        if send_response[0] == "REGISTERED" and recv_response[0] == "REGISTERED":
            print("\n" + send_response[2] + ", you have been registered at the server.")
            print("To start a conversation, tag a user. Tag ALL to broadcast. To end application, type CLOSE.\n")
