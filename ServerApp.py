...
def sendReqAccept(reg_req, conn_socket):
...
def recvReqAccept(reg_req, conn_socket):
...
while True:
    ...
    if reg_req[0] != "REGISTER":
        ...
        thr = threading.Thread(target = sendReqAccept, args = (reg_req, req))
        threads.append(thr)
        thr.start()
    else:
        thr = threading.Thread(target = recvReqAccept, args = (reg_req, req))
        threads.append(thr)
        thr.start()
